...
class Auction(Application):
    ...
    highest_bid: Final[ApplicationStateValue] = ApplicationStateValue(
        stack_type=TealType.uint64,
        default=Int(0),
        descr="The value of the highest bid"
    )
    ...
    def opt_into_asset(self, asset: abi.Asset):
        return Seq(
            ...
            InnerTxnBuilder.Execute({
                ...
                TxnField.fee: Int(0), # Let the end user to pay for the fee instead of the smart contract itself
            })
        )
    ...
    def start_auction(
        ...
        length: abi.Uint64,
        axfer: abi.AssetTransferTransaction # axfer = shortcut to say asset transfer
    ):
        return Seq(
            Assert(self.auction_end == Int(0)),
            Assert(axfer.get().asset_receiver() == Global.current_application_address()),
            self.asa_amt.set(axfer.get().asset_amount()), # setting the asset amount and making sure everyone is bidding for the same one
            self.auction_end.set(length.get() + Global.latest_timestamp()), # end time must be the same for everyone
    ...
    def bid(self, payment: abi.PaymentTransaction, previous_bidder: abi.Account):
        return Seq(
            Assert(Global.latest_timestamp() < self.auction_end), # check the auction hasn't ended
            Assert(payment.get().amount() > self.highest_bid), # check amount is higher than highest bid
            Assert(payment.get().receiver() == Global.current_application_address()), # verify payment has been received
            Assert(Txn.sender() == payment.get().sender()), # verify sender of transaction is the same as the one from payment
    ...
    def delete(self):
        return InnerTxnBuilder.Execute({
            ...
            TxnField.close_remainder_to: Global.creator_address(), # sending 0.1 (min required for an account) back to the creator
